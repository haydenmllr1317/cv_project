...
def test_model(model_type,model_path):
    """
    Tests the perturbation resiliance of a given model, and outputs the relevant graphs

    Args:
        model_type (string): Either "CLIP" or "UNet"
    ...
    for perturb_index in range(len(perturbNames)):
        ...
        os.makedirs(f"perturbEval/{model_type}/",exist_ok=True)
        ...
        plt.plot(perturbParamSweeps[perturb_index], hyperParam_results, marker='o', linestyle='-', label=f"{model_type} Model Dice Score")
        ...
        plt.tight_layout()
        os.makedirs(f"perturbEval/{model_type}/Run0",exist_ok=True)
        plt.savefig(f'perturbEval/{model_type}/{perturbNames[perturb_index]}.png',bbox_inches='tight', pad_inches = 0.05)
...
"""
Examples:
    pythn perturbEval.py --model_type CLIP --model_path Runs/Clip/Run0/Checkpoints/gs3349_e16.safetensors
"""
if __name__ == '__main__':
    #Create parser
    parser = argparse.ArgumentParser(description='Tests the perturbation resilience of a model and generates graphs')
    ...
    parser.add_argument(
        ...
        required=True,
        choices=['CLIP', 'UNet'],
        help='Type of the model (CLIP or UNet)'
    )
    parser.add_argument(
        '--model_path',
        required=True,
    ...
    with torch.no_grad():
        test_model(args.model_type, args.model_path)
